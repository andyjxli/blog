...
> Rust 中既没有运行时的概念，也没有垃圾回收（JS、Java）的机制。而是通过**所有权**来保证运行的高效与内存的利用率。
>
> **所有权将内存检查提前到了编译阶段**，保证了内存安全的同时也保证了运行时速度。
...
理解起来很简单，每声明一个变量就是声明一个所有者，是其值的所有者。当这个所有者离开当前作用域的时候，值就会被丢弃。
...
> 对于基本数据类型来说，移动是没有任何意义的，本身就是在栈上的数据类型，所以自身就能提供 clone trait。而对于复杂数据类型例如 String 类型，需要通过 clone 来显式的进行克隆，否则就是移动.
...
函数的参数以及函数的返回值其实是根据其值的类型来决定是移动还是复制的，即其所有权由值的类型来决定。 
